...
void tGeneralPlayerEntity::tGeneralPlayerEntityFunctions::GetGeneralPlayerEntityInfo(tGeneralPlayerEntityInfo* GeneralPlayerEntityInfo, int PlayerNumber)
{
	if (GeneralPlayerEntityInfo)
	{
		...
		{
			pIO.Functions.Read<DWORD>(&GeneralPlayerEntityInfo->BaseAddress, pIO.ClientModuleBaseAddress + pGlobalVars.Offsets.Client + pGlobalVars.Offsets.m_dwEntityList);// ÄÎÄÅËÀÒÜ!!!!  // + (pGlobalVars.Offsets.EntitySize * PlayerNumber));
